    ...
    <el-row :gutter="40">
      ...
      <el-col :xs="12" :sm="12" :lg="6">
        <div class="basic-item single-date">
          <label>首期入伙时间：</label>
          <el-date-picker v-model="firstEntryDate" type="date" placeholder="选择日期"></el-date-picker>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="40">
...
export default {
  ...
  methods: {
    add() {
      ...
      if (this.firstEntryDate) {
        this.mainMsg.firstEntry = parseTime(this.firstEntryDate, '{y}-{m}-{d}')
      } else {
        this.mainMsg.firstEntry = ''
      }
      // console.log('mainMsg', this.mainMsg.firstEntry)
